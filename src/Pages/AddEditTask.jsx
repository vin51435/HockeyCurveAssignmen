      ...
      <div className='w-full sm:w-2/4'>
        ...
        <div className='block p-4 rounded-md bg-zinc-100'>
          <div className='pb-4 flex justify-between items-center text-xl'>
            ...
            <Link to={'/'} className='flex px-8 py-1 h-full bg-rose-600 text-sm font-medium text-white rounded-3xl'>Delete</Link>
          </div>
          <form className='flex flex-col gap-2' onSubmit={(e) => submitHandler(e)}>
            ...
            <div className={`${showOptions ? 'opacity-1' : 'opacity-0 pointer-events-none'} flex py-1 font-medium flex-col -top-1 justify-space items-start relative w-full shadow-[inset_0_0_4px_0_rgba(0,0,0,0.5)] shadow-gray-400 bg-white`}>
              <div
                onClick={() => {
                  setForm(prev => ({ ...prev, priority: 1 }));
                  setShowOptions(false);
                }}
                className='flex justify-between items-center w-full px-3 py-1 cursor-pointer border-b-2 border-emerald-800'>
                <span>High</span>
              ...
              <div
                ...
                className='flex justify-between items-center w-full px-3 py-1 cursor-pointer'>
                <span>Low</span>
            ...
            </div>
            {
              type === 'add' ?
                <div className='flex justify-between items-center gap-4 py-9'>
                  <button
                    type='submit'
                    className='flex flex-1 items-center justify-center px-8 py-1 h-full bg-emerald-800 text-sm font-medium text-white rounded-3xl cursor-pointer '>
                    Add Task
                  </button>
                  <span className='flex flex-1 items-center justify-center px-8 py-1 h-full bg-emerald-800 text-sm font-medium text-white rounded-3xl cursor-pointer '>
                    <Link to={'/'}>Cancel</Link>
                  </span>
                </div>
                :
                <div className="grid grid-cols-2 gap-4 py-3">
                  <button
                    type='submit'
                    className="flex justify-center cursor-pointer col-span-1 py-1 h-full bg-emerald-800 text-sm font-medium text-white rounded-3xl">
                    Save Changes
                  </button>
                  <button
                    type='button'
                  ...
                  <Link to={'/'} className="flex justify-center col-span-2 mx-auto px-8 py-1 h-full bg-emerald-800 text-sm font-medium text-white rounded-3xl">
                    Cancel
                  </Link>
                </div>
            }
